...
function decodeHexString(value: unknown) {
...
function deriveDefuseSubkey(rawKey: Buffer, salt: Buffer, info: Buffer) {
  return Buffer.from(crypto.hkdfSync("sha256", rawKey, salt, info, DEFUSE_KEY_BYTES));
}

export function decryptLicenseKey(encryptedHex: string) {
  try {
